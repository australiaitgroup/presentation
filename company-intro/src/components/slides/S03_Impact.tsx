...
const data = [
	{ num: '6,000+', label: 'Offers', desc: '学员拿到工作', bg: colors.red, color: '#fff', rotate: -3, scale: 1.05 },
...
export default function S03_Impact() {
	return (
		<Slide bg={colors.warmBg} style={{ position: 'relative' }}>
			{/* Animated backgrounds — layered for visual richness */}
			<AnimatedBg variant="shapes" opacity={0.12} />
			<AnimatedBg variant="dots" opacity={0.06} />

			{/* Top color stripe */}
			<div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 8, zIndex: 2, background: `repeating-linear-gradient(90deg, ${colors.red} 0, ${colors.red} 25%, ${colors.yellow} 25%, ${colors.yellow} 50%, ${colors.green} 50%, ${colors.green} 75%, ${colors.blue} 75%, ${colors.blue} 100%)` }} />
			...
			<div style={{ position: 'relative', zIndex: 1, width: '90%', maxWidth: 1300, padding: 40 }}>
				...
				<motion.h2
					...
					style={{
						...
						fontWeight: 900, color: colors.black, lineHeight: 1, marginBottom: 40,
					}}
				>
					我们的<br />
					<span style={{
						display: 'inline-block', background: colors.red, color: '#fff', padding: '0 24px',
						border: `4px solid ${colors.black}`, transform: 'rotate(-1deg)',
					...
					>
						...
						<div style={{ marginTop: 20, display: 'flex', gap: 16, flexWrap: 'wrap', justifyContent: 'center' }}>
							{['ATLASSIAN', 'Canva', 'aws', 'Deloitte', 'Google', 'Microsoft'].map(p => (
								<span key={p} style={{ fontFamily: fonts.mono, fontSize: 14, fontWeight: 700, color: 'rgba(0,0,0,0.3)' }}>{p}</span>
							))}
						</div>
					</motion.div>
				</div>
